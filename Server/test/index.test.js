...
describe("Test de RUTAS", () => {
    describe("GET /rickandmorty/character/:id", () => {
        ...
        it("Si hay un error responde con status: 500", async ()=>{
            await agent.get('/rickandmorty/character/3451').expect(500);
        })
    });
    
    describe("GET /rickandmorty/login",() => {
       it("Retorna un objeto con la propiedad access en true", async () => {
        const response = await agent.get("/rickandmorty/login?email=cataldof21@gmail.com&password=1234567");
        expect(response.body.access).toBe(true);
       }) 
       it("Retorna un objeto con la propiedad access en false si hay error", async () => {
        const response = await agent.get("/rickandmorty/login?email=cataldof21@gmail.com&password=1234");
